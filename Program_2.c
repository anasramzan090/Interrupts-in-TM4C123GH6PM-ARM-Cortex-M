...
# define GPIO_PORTF3_DATA_WR (*(( volatile unsigned long *)0x40025020)) 
# define GPIO_PORTF1_DATA_WR (*(( volatile unsigned long *)0x40025008)) 
...
int main(void)
{
  ...
  GPIO_PORTF_LOCK_R |= 0;/* lock commit register */
    ...
    GPIO_PORTF_DIR_R |=GPIO_PORTF_PIN3_EN + GPIO_PORTF_PIN1_EN;/* Set PF3, PF1 as digital output to control green and RED LED */
    GPIO_PORTF_DEN_R|= GPIO_PORTF_PIN3_EN + GPIO_PORTF_PIN4_EN + GPIO_PORTF_PIN0_EN + GPIO_PORTF_PIN1_EN ;/* make PORTF4-0 digital pins */
    GPIO_PORTF_PUR_R |= GPIO_PORTF_PIN4_EN |(GPIO_PORTF_PIN0_EN );/* enable pull up for PORTF4, 0 */
    
    /* configure PORTF4, 0 for Rising edge trigger interrupt */
   ...
   GPIO_PORTF_IEV_R |= (GPIO_PORTF_PIN4_EN)+ (GPIO_PORTF_PIN0_EN );/* Rising edge trigger */
...
{	
  ...
  if (GPIO_PORTF_MIS_R & 0x10) /* check if interrupt causes by PF4/SW1*/
    {   
			 GPIO_PORTF1_DATA_WR  &= ~GPIO_PORTF_PIN1_EN;
      GPIO_PORTF3_DATA_WR|= GPIO_PORTF_PIN3_EN; //TURN on green LED
     GPIO_PORTF_ICR_R|= 0x10; /* clear the interrupt flag */
     } 
    else if (GPIO_PORTF_MIS_R & 0x01) /* check if interrupt causes by PF0/SW2 */
    { 
     GPIO_PORTF3_DATA_WR&= ~GPIO_PORTF_PIN3_EN;			
     GPIO_PORTF1_DATA_WR  |= GPIO_PORTF_PIN1_EN; // Turn on RED LED
